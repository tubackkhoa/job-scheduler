...
def fetch_positions_with_pnl(
    base_url: str, api_key: str, start_time: Optional[str] = None
) -> List[Dict[str, Any]]:
    ...
    try:
        if start_time:
            response = httpx.get(
                url, headers=headers, params={"startTime": start_time}, timeout=30.0
            )
        else:
...
def get_signal_comparison(
    ...
    try:
        ...
        for job_info in matched_jobs:
            df = extract_signals_from_job(job_info["job_id"], config.signal_keyword)
        ...
        if "pred_time" not in combined_df.columns or "base_asset" not in combined_df.columns:
            return pd.DataFrame(
                {"message": ["Missing required columns (pred_time or base_asset)."]}
            )
        ...
        positions = fetch_positions_with_pnl(
            config.webhook_url, config.webhook_api_key, start_time_iso
        )
