...
def register_model_config(payload: dict[str, str]) -> bool:
    """
    ...
    default_config = {"tp_pct": 2.5, "sl_pct": 1.7, "start_date": datetime.now().timestamp()}

    model = ForwardTestPluginModel(model_name=model_name, config=default_config)
    ...
    if "start_date" in db_config:
        ...
        if isinstance(start_date_val, str):
            try:
                ...
                logger.error(
                    f"Invalid date format for start_date: {start_date_val}. Expected YYYY-MM-DD."
                )
                pass
...
def get_running_models() -> List[Dict[str, Any]]:
    ...
    for m in models:
        ...
        result.append({"identity": m.model_name, "currentConfig": display_config})
...
def get_equity_curve_forward_test_fromdb(
    base_url: str, api_key: str, identity: str, start_time: Optional[str], end_time: Optional[str]
) -> List[dict]:
    """
