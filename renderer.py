...
def tojson(obj, *, indent=None, sort_keys=False):
    data = ms.encode(
        obj,
        order="sorted" if sort_keys else None,
    )
    # If pretty-print requested, format afterward
    if indent is not None:
        data = ms.format(data, indent=indent)
...
class Renderer:
    ...
    _sandbox.policies["json.dumps_function"] = tojson
